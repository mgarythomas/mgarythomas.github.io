...
This means your feedback loops had better be fast, or you'll find yourself explaining to stakeholders why the AI decided that deleting all customer data was "probably what you meant."
...
XP's focus on teamwork hasn't been made redundant by AI, it's been clarified, where the 'pair' is now a human and an enthusiastic assistant that never needs coffee breaks, never complains about your code or coding ability, and hasn't yet learned to passive-aggressively update JIRA tickets.
...
* **Pipeline as Gatekeeper**: Every commit flows through a robust CI/CD pipeline that validates thoroughly and as quickly as possible (being able to commit frequently is predicated by a build pipeline that can execute in minutes)
...
Progress, I suppose, comes in many forms—even when it looks suspiciously like re-inventing the past.